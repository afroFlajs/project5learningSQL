...
const mostSeeasonWins = (season, country) => {
...
const mostSummerWins = country => {
  return mostSeeasonWins('Summer',country);
};
...
const mostWinterWins = country => {
  return mostSeeasonWins('Winter', country);
};

const countryWithBestCount = (bestThing, country) => {
  if (['year', 'discipline', 'sport', 'event'].includes(bestThing)) {
    return `SELECT &{bestThing}, 
    COUNT (*)AS count FROM GoldMedal WHERE country =
    '${country}' GROUP BY ${bestThing} ORDER BY COUNT(*) DESC
    LIMIT 1;`;
  }
  return null;
};
...
const numberGenderMedalists = (gender, country) => {
  if (['Men', 'Women'].includes(gender)) {
    ...
    FROM GoldMedal WHERE country = '${country}' AND gender = '${gender}';`;
  }
  return null;
};
...
const orderedMedals = (country, field, sortAscending) => {
  ...
  if (field) {
    if (sortAscending) {
      orderingString = `ÒRDER BY ${field} ASC`;
    } else {
      orderingString = `ÒRDER BY ${field} DESC`;
    }
  }
